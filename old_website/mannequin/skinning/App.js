...
export class SkinningAnimation extends CanvasAnimation {
  ...
  initSkeleton() {
    ...
    this.skeletonRenderPass.addUniform("bHighlight", (gl, loc) => {
      gl.uniform1fv(loc, this.getScene().meshes[0].getBoneHighlights());
    });
  ...
  updateSkeletonState() {
    let time = this.getGUI().time;
    vue.playheadTime = time; // update timeline
    let currentMesh = this.scene.meshes[0];
    let keyFrames = [...currentMesh.keyFrames].sort((a, b) => {
      return a.time - b.time;
    });
    // which frames do we interpolate between?
    let currentIndex = -1;
    let nextIndex = -1;
    let length = -1;
    for (let i = 0; i < keyFrames.length - 1; i++) {
      if (keyFrames[i].time < time && keyFrames[i + 1].time > time) {
        currentIndex = i;
        nextIndex = i + 1;
        length = keyFrames[i + 1].time - keyFrames[i].time;
      }
    }
    // accessing the two frames selected
    let currentKeyFrame = keyFrames[currentIndex].rotations;
    let nextKeyFrame = keyFrames[nextIndex].rotations;
    // updating all the bones at all time steps
    for (let i = 0; i < currentKeyFrame.length; i++) {
      let bone = currentMesh.bones[i];
      bone.T = Quat.slerpShort(
        currentKeyFrame[i],
        nextKeyFrame[i],
        (time - keyFrames[currentIndex].time) / length
      );
      // updating root bone for forward kinematics
      if (bone.parent == -1) {
        bone.rotation = bone.T;
        ...
        this.gui.updateChildBones(bone);
      }
    }
  }
  /** @internal
...
export function initializeCanvas() {
  ...
  canvasAnimation.setScene("/mannequin/static/assets/skinning/split_cube.dae");
